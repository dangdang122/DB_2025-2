...
select 고객이름, 주문제품, 주문일자 from 주문 left outer join 고객 on 고객아이디=주문고객;

-- Sub Query
-- 달콤비스킷을 생산한 제조업체가 만든 제품들의 제품명과 단가를 검색하시오.
select 제품명, 단가 from 제품 where 제품명 = '달콤비스킷';
...
select 고객아이디, 나이, 적립금, 제품명 단가 from 고객,제품,주문 where 고객아이디 = 주문고객 and 제품번호= 주문제품;
select 고객아이디, 나이, 적립금, 제품명, 단가 from 고객, 제품, 주문 where 고객아이디 = 주문고객 and 제품번호= 주문제품 and 주문고객 in(select 고객아이디 from 고객 where 고객이름 like '김%');


-- SubQuery 다중행결과에 사용하는 Not IN연산자
...
-- 2022년 3월 15일에 제품을 주문한 고객의 고객이름 검색.
select * from 주문;
