...
select 주문고객, round (avg(수량),1) "수량의 평균" from 주문 group by 주문고객 order by 주문고객 desc;
-- select round (avg(단가),2) as 단가평균 from 제품;

--고객 테이블에서 등급별 적립금의 평균을 고객아이디를 기준으로 오름차순 검색
select 등급, round (avg(적립금),2) "등급별 적립금의 평균" from 고객 group by 등급 order by 등급;
...
-- 고객 테이블에서 직업별 나이의 평균과 가장 작은 적립금을 검색하시오. (단, 나이의 평균은 평균나이, 가장 작은 적립금 최저 적립금이라는 컬럼명으로 출력)
...
--제품 테이블에서 [제품을 2개 이하 제조한] 제조업체별로 제조한 제품의 개수와 제품중 가장 비싼 단가중 [4000원 이상인것을] 검색하시오. (단, 제품의 개수는 제품수, 가장 비싼 단가는 최고가라는 컬럼명으로 출력)
select 제조업체, count(*) 제품수, max(단가) 최고가 from 제품 group by 제조업체 having count(*)<=2 and max(단가)>= 4000;

--고객 테이블에서 [적립금 평균이 1000원 이상인 등급에 대해] 등급별 고객수와 적립금 평균을 검색하시오.
-- 단, 고객수는 고객수로 적립금 평균은 평균은 평균 적립금으로 출력, 평균 적립금은 소수점 이하 둘째자리까지
select 등급, count(*) 고객수, round(avg(적립금),2) "평균적립금" from 고객 group by 등급 having max(적립금)>=1000;


--주문테이블에서 각 주문고객이 주문한 제품의 총주문수량을 주문제품별로 검색하시오.
-- 단, 주문제품, 주문고객, 총주문수량을 컬럼으로 출력하고 총 주문 수량이 30개 이상, 주문 제품이 p01~p03까지의 제품만 출력
...
select 고객이름, 나이, 직업, 제품명, 단가, 주문일자, 배송지 from 주문, 고객, 제품 where 주문고객='apple' and 주문고객=고객아이디 and 주문제품=제품번호;
