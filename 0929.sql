select * from 고객;
-- 고객테이블에 고객아이디는 strawberry, 고객 이름 최유경, 나이 30세, 등급 vip, 직업은 공무원, 적립금 100원인 고객의 정보를 삽입(추가)하시오.
-- 속성(컬럼, 필드) 리스트를 생략하여 추가
insert into 고객 values('strawberry', '최유경', 30, 'vip', '공무원',100);

-- 고객테이블에 고객아이디는 kiwi, 고객 이름 김키위, 나이 25세, 등급 gold, 직업은 회사원, 적립금 500원인 고객의 정보를 삽입(추가)하시오.
insert into 고객(고객아이디, 고객이름, 직업, 나이, 등급, 적립금) values('kiwi', '김키위', '학생',23, 'silver', 500);

-- 고객 테이블에 고객아이디는 grapes, 고객이름강포도, 등급 vip, 직업은 의사, 적립금 1500원인 고객의 정보를 삽입(추가)하시오.
...
-- 정소화 고객이 주문한 제품의 주문 수량을 5개로 수정하시오.
...
--제품테이블에서 제조업체별 제품수로 구성된 업체별 제품수라는 이름의 view를 생성해보자.
